...
public class CommandLineRemoteRunner {
    ...
    private boolean parseCommandLine(String[] args) throws ParseException, IOException, LivingDocServerException {
        cli.parse(args);
    ...
    private void defineCommandLine() {
        ...
        cli.defineOption(cli.buildOption("user", "--user USER", "Confluence user name"));
        cli.defineOption(cli.buildOption("password", "--password PASSWORD", "Confluence password"));
        cli.defineOption(cli.buildOption("config", "--config CONFIG", "Confluence credentials file path"));
    }
