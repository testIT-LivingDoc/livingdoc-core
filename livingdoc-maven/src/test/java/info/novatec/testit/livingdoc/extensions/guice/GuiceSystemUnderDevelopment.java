...
public class GuiceSystemUnderDevelopment extends DefaultSystemUnderDevelopment {
    ...
    public Fixture getFixture(String name, String... params) throws Throwable {
        ...
        if (params.length != 0) {
            // When params are used, we use the PlainOldSystemUnderDevelopment
            // to instantiate the fixture
    ...
    private void lazilyInstantiateModulesAndInjector() throws InstantiationException, IllegalAccessException {
        if (injector == null) {
    ...
    public void addModules(String... moduleNamesToAdd) {
        if (injector == null) {
            Collections.addAll(this.moduleNames, moduleNamesToAdd);
        } else {
