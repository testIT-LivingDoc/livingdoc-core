...
public class DecisionTableInterpreter extends AbstractInterpreter {
    ...
    private List<Column> parseColumns(Example table) {
        ...
        if (headers == null) {
            List<Column> newColumn = new ArrayList<Column>(0);
            LOG.trace(EXIT_WITH, newColumn.toString());
            return newColumn;
        }

        List<Column> columns = new ArrayList<Column>(headers.remainings());
        for (int i = 0; i < headers.remainings(); i ++ ) {
           Example anExample = headers.at(i); 
           Column aColumn = parseColumn(anExample);
           columns.add(aColumn);
