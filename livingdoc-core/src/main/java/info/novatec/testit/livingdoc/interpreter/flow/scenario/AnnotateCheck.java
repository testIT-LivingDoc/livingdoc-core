...
class AnnotateCheck implements Stub {
    ...
    AnnotateCheck(Annotatable annotable, Statistics stats) {
    ...
    public void call(Result result) {
        ...
        if (result.isException()) {
            ...
            Boolean actual = ( Boolean ) result.getActual();

            if (actual) {
                statistics.right();
                annotable.annotate(Annotations.right());
            } else {
                statistics.wrong();
                annotable.annotate(Annotations.wrong());
            }
        }
    }
}
